...
### 🔖 智慧功能 (NEW!)
- **IP 歷史記錄** - 自動記住最近使用的 10 個 IP 位址
- **鍵盤快捷鍵** - Ctrl+C 連線、F5 測試、Ctrl+S 儲存等
- **CSV 匯出** - 將測試結果匯出為 Excel 可讀取的 CSV 格式
- **改進的錯誤提示** - 更詳細的錯誤訊息和解決建議
...
- 所有測試結果會即時顯示在日誌區域
- 點擊「儲存日誌」保存測試記錄（Ctrl+S）
- 點擊「匯出 CSV」將結果匯出為 Excel 格式
- 性能測試會生成詳細的報告檔案
...
想了解如何建立完整的 Modbus 測試項目？請參考：

👉 [Modbus 測試項目建立指南](Modbus測試項目建立指南.md)

這份指南包含：
- 完整的 Modbus 位址表和功能碼說明
- 每個測試項目的詳細測試腳本範例
- 測試優先級和檢查清單
- 測試報告範本
...
├── README.md                   # 專案說明（本文件）
├── 專案規格文件.md              # 完整技術規格
├── Modbus測試項目建立指南.md    # 測試項目建立指南
├── 使用說明.txt                 # 詳細使用說明
├── 功能說明.txt                 # 功能詳細說明
├── Release/                    # 發布資料夾（給同事使用）
│   ├── TMflow_Modbus_Testkit.exe
│   ├── 使用說明.txt
│   ├── 功能說明.txt
│   └── README.txt
└── .gitignore                  # Git 忽略規則
...
### v1.0.2.0003 (2026-02-11) - 使用體驗大幅提升
- ✅ 新增測試套件功能（批次執行多個測試）
- ✅ 新增 IP 歷史記錄（自動記住常用 IP）
- ✅ 新增鍵盤快捷鍵（Ctrl+C, F5, Ctrl+S 等）
- ✅ 新增 CSV 匯出功能（Excel 相容）
- ✅ 改進錯誤提示（更詳細的解決建議）
- ✅ 新增設定檔自動儲存
- ✅ 優化使用者介面和操作流程
